...
void main() {
  ...
  String statusMessage = switch (response) {
  ...
  print('Response Message: $statusMessage');
  ...
  for (int outer = 1; outer <= 3; outer++) {
    for (int inner = 1; inner <= 3; inner++) {
      if (outer * inner == 4) {
        print('Breaking outerLoop at outer=$outer, inner=$inner (product is 4)');
        break outerLoop;
      }
      print('Processing outer=$outer, inner=$inner');
    }
  }

  // 5. Advanced assert Statements
  int userAge = 20; // Set to >= 18 to avoid throwing AssertionError in execution
  assert(
    userAge >= 18,
    "Access denied: User is $userAge years old, minimum required is 18.",
  );
  print('Age verification passed for user ($userAge years old)');
}